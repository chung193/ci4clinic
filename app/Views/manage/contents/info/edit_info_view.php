...
<section class="content">
    ...
    <form action="<?php echo base_url()?>/manage/info/update" method="post" enctype='multipart/form-data'>
        <div class="mb-3">
            ...
            <input type="text" id="title" class="form-control" name="name" placeholder="Tên shop" value="<?php echo $info->name?>">
        </div>
        <div class="mb-3">
            <label for="title" class="form-label">Địa chỉ</label>
            <input type="text" class="form-control" name="address" placeholder="meta title" value="<?php echo $info->address?>">
        </div>

        <input type="hidden" class="form-control" name="id" value="<?php echo $info->id?>">
        <input type="hidden" class="form-control" name="logo_old" value="<?php echo $info->logo?>">
        <input type="hidden" class="form-control" name="favicon_old" value="<?php echo $info->favicon?>">
        
        <div class="mb-3">
            <label for="title" class="form-label">Điện thoại</label>
            <input type="text" class="form-control" name="phone" placeholder="phone" value="<?php echo $info->phone?>">
        </div>

        <div class="mb-3">
            <label for="title" class="form-label">Email</label>
            <input type="text" class="form-control" name="email" placeholder="email" value="<?php echo $info->email?>">
        </div>
        ...
        <?php if($info->logo){ ?>
                <img src="<?php echo base_url().'/public/uploads/logo/'.$info->logo?>" width="200">
        <?php } ?>
        </div>
        ...
        <?php if($info->favicon){ ?>
                <img src="<?php echo base_url().'/public/uploads/favicon/'.$info->favicon?>" width="100" style="margin: 0 20;display: block">
        <?php } ?>
